...
var currentMode : number = data.mode; // Get mode
var currentEventLength : number = data.length; // Get event length
...
$.initializeDateCalendar = function(){
	...
	for (i=0;i<daysInMonth;i++) {
		...
		if ($.inArray(date, availableArray) != -1) {
			dateListItem = $('<li class="ui-state-default">'+currentDateIndex+'</li>');
		}
		else
		{
			dateListItem = $('<li class="ui-state-disabled">'+currentDateIndex+'</li>');
		}
...
$.initializeTimeCalendar = function() {
	...
	for (var timeIndex=1; timeIndex<=UNITS_IN_DAY; timeIndex++) {
		...
		for (var weekdayIndex=0; weekdayIndex<DAYS_IN_WEEK; weekdayIndex++) {
			var timeListItem = $('<li class="ui-state-disabled"></li>');
...
$.resetTimeCalendar  = function() {
	...
	for (var i=1; i<=UNITS_IN_DAY;i++) {
		for (var j=0;j<DAYS_IN_WEEK;j++) {
			...
			if ($.inArray(startTimeIndex+time, availableArray) == -1) {
				$('li[time='+time+']').removeClass('ui-state-default');
				$('li[time='+time+']').removeClass('ui-selectee');
				$('li[time='+time+']').addClass('ui-state-disabled');
			}
			else{
				$('li[time='+time+']').removeClass('ui-state-disabled');
				$('li[time='+time+']').addClass('ui-state-default');
				$('li[time='+time+']').addClass('ui-selectee');
			}
...
$('#create').click(function() {
	...
	// If selected, calculate and return results
	else {
