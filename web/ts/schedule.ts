...
var dragIndexStart : number;
var dragIndexEnd : number;
...
var dragStartColumn : number;
var dragStartRow : number;
var dragEndColumn : number;
var dragEndRow : number;
...
$('#calendar').on('mousedown', '.time.enabled', function(event) {
	...
	if ($.inArray(startTimeIndex+dragStartColumn*UNITS_IN_DAY+dragStartRow, availableArray) != -1) { // If date is available
		// Starts drag
		isCalendarMouseDown = true;
...
.on('mouseover', '.time.enabled', function(event) {
	...
	var dragNewEndColumn : number = parseInt($(this).attr('column'));
	var dragNewEndRow : number = parseInt($(this).attr('row'));
	var startTimeIndex =  (currentWeekIndex-startWeekIndex)*DAYS_IN_WEEK*UNITS_IN_DAY; // Get start time index of current week
	// If user is currently dragging
	if (isCalendarMouseDown && dragNewEndRow>=dragStartRow && dragNewEndColumn>=dragStartColumn) {
		...
		for (var j=dragEndColumn;j<=dragNewEndColumn;j++) {
			for (var i = dragStartRow;i<=dragNewEndRow;i++) {
				if ($.inArray(startTimeIndex+j*UNITS_IN_DAY+i, availableArray) != -1) { // If date is available
					if (isAdding) {
