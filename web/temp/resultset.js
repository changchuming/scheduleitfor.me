    ...
    for (var i = 0; i < resultset.length; i += 2) {
        ...
        if (resultset[i + 1] != resultset[i + 3]) {
            ...
            for (var j = 0; j < dateStoreArray.length; j++) {
                var percent = Math.round(resultset[i + 1] / parseInt(usercount) * 100);
                ...
                var bartext = $('<div id="bartext">' + resultset[i + 1] + '/' + parseInt(usercount) + ' (' + percent + '%)' + '</div>');
                bartext.css('width', percent + '%');
                var bar = $('<div id="bar"/>').progressbar({ value: percent });
                bar.append(bartext);
